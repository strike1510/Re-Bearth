import pygame, sys , jeu.room1 , menu.pause
def LancementJeu(screen,pos_player_x,pos_player_y):
    testpause = False
    WHITE = (255, 255, 255)
    BLACK = (0, 0, 0)
    TRANSPARENT = (0, 0, 0, 0)
    ...
    porte_rect = porte.get_rect(center=(1600, 1080 // 2))
    ...
    while running:
        ...
        if player_rect.x > 1545:
            if player_rect.x < 1580:
                if player_rect.y < 515:
                    if player_rect.y > 470:
                        image_porte = image_porte_open
                        if key[pygame.K_SPACE] == True:
                            jeu.room1.Jeuroom1(screen,270,1080 // 2)
                            running = False
                    else:
                        image_porte = image_porte_close
                else:
